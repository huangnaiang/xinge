...
function XingeApp(accessId, secretKey){
	 ...
	 */
	this.pushToSingleDevice = function(deviceToken, message, callback){
	 ...
	 * @param  {Function} callback(err, result) 回调函数
	 */
	this.pushToSingleAccount = function(deviceType, account, message, callback){
	 ...
	 * @param  {Function} callback(err, result) 回调函数
	 */
	this.pushToAllDevices = function(deviceType, message, callback){
	 ...
	 * @param  {Function} callback(err, result) 回调函数
	 */
	this.pushByTags = function(deviceType, tags, tagOperation, message, callback){
	...
	this.queryTags = function(start, limit, callback){
		...
		//调用API
		callAPI(API_QUERY_TAGS, params, "POST", this.secretKey, callback);
...
function Message(){
	...
	this.format = function(){
		...
		if(!isEmptyObj(this.customContent)){
			//mess.custom_content = JSON.stringify(this.customContent);
			mess.custom_content = this.customContent;
		}

		if(this.acceptTime.length > 0){
			//mess.accept_time = JSON.stringify(this.acceptTime);
		...
		if(this.type === exports.MESSAGE_TYPE_NOTIFICATION){
			...
			//mess.action = JSON.stringify(this.action);
...
function callAPI(api, params, method, secretKey, callback){
	...
	try{
		...
		//console.log('------------');
		//console.log(encodeURIComponent(params.message));
